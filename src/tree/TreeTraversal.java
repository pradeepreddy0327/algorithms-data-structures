...
public class TreeTraversal {
    ...
    public static void main(String[] args) {
        ...
        TreeNode copy = obj.constructTree(new int[] { 4, 2, 5, 1, 6, 3, 7 },
                new int[] { 4, 5, 2, 6, 7, 3, 1 }, 0, 7, 0, 7);
        TreeNode copy2 = obj.constructTree(new int[] { 9, 3, 15, 20, 7 },
                new int[] { 9, 15, 7, 20, 3 }, 0, 5, 0, 5);
    ...
    private TreeNode constructTree(int[] inorder, int[] postorder, int ib,
            int ie, int pb, int pe) {
        System.out.println(
                " ib :" + ib + " ie :" + ie + " pb :" + pb + " pe :" + pe);
        if (ib < ie && pb < pe) {
            int val = postorder[pe - 1];
            TreeNode head = new TreeNode(val);
            int index = find(inorder, val);
            head.left = constructTree(inorder, postorder, ib, index, pb,
                    pb + (index - ib));
            head.right = constructTree(inorder, postorder, index + 1, ie,
                    pe - ie + index, pe - 1);
            return head;
        }
        return null;
